...
-- DELETE
CREATE EXCEPTION EX_PD_EN_USO
'No se puede eliminar el detalle porque tiene transacciones asociadas';

SET TERM ^ ;

CREATE PROCEDURE SP_PD_DELETE (
    P_ID_DETALLE INTEGER
)
AS
DECLARE VARIABLE V_TMP INTEGER;
DECLARE VARIABLE V_SUB INTEGER;
DECLARE VARIABLE V_PRES INTEGER;
BEGIN
    SELECT ID_SUBCATEGORIA, ID_PRESUPUESTO
    FROM PRESUPUESTO_DETALLE
    ...
    SELECT 1
    FROM TRANSACCION
    WHERE ID_SUBCATEGORIA = :V_SUB
      AND ID_PRESUPUESTO = :V_PRES
    ROWS 1
    INTO :V_TMP;

    IF (V_TMP IS NOT NULL) THEN
        EXCEPTION EX_PD_EN_USO;
